...
def retrain_model_imoex(**kwargs):
    ...
    try:
        remove('data/for_catboost_imoex.csv')
    except:
        pass
    s3_hook.download_file(
    ...
    try:
        remove('data/yesterday_query_imoex.csv')
    except:
        pass
    s3_hook.download_file(
        key = "yesterday_query_imoex.csv",
        bucket_name = "studcamp-ml",
        local_path = 'data',
        preserve_file_name = True,
        use_autogenerated_subdir = False
    ...
    q = pd.read_csv('data/yesterday_query_imoex.csv')
    ...
    imoex_filled = pd.read_csv('data/IMOEX_filled.csv')

    X = df.drop(columns=['open'])
    y = df['open']
    cb_reg.fit(X,y)
    joblib.dump(cb_reg, "data/catboost_imoex.joblib")
    ...
    today_query['open'] = imoex_filled.tail(1).open.values[0]
    today_query.to_csv('data/today_query.csv',index=False)
    s3_hook.load_file('data/today_query.csv', key = "yesterday_query_imoex.csv", 
                        bucket_name="studcamp-ml", replace=True)
    remove('data/for_catboost_imoex.csv')
    remove('data/yesterday_query_imoex.csv')
    remove('data/IMOEX_filled.csv')
...
def model_predict_imoex(**kwargs):
    ti = kwargs["ti"]
    info = ti.xcom_pull(task_ids="retrain_model_imoex")
...
def save_last_hundred_imoex(**kwargs):
    ti = kwargs["ti"]
    info = ti.xcom_pull(task_ids="model_predict_imoex")
    ...
    df_filled = pd.read_csv("data/IMOEX_filled.csv").tail(99)
